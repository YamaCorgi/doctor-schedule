...
def get_calendar_service(target_email):
    """取得 Google Calendar 服務。每個 email 會用獨立的 token 檔案。"""
    # 用 email 產生獨立的 token 檔名，支援多個帳號
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            # login_hint 會讓 Google 授權頁面自動選擇該帳號
...
def write_to_calendar(shifts, doctor_name, target_email):
    ...
    try:
        ...
        if actual_email.lower() != target_email.lower():
            return 0, f"登入的帳號（{actual_email}）與目標（{target_email}）不符，請重新授權"
